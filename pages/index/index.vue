			...
			<view class="swiper-box">
				<swiper circular="true" autoplay="true" @change="swiperChange">
					<swiper-item v-for="swiper in swiperList" :key="swiper.id">
						<image :src="swiper.image_url" @tap="toSwiper(swiper)"></image>
					</swiper-item>
				</swiper>
	...
	export default {
		...
		data() {
			return {
				...
				swiperList: [{
						id: 1,
						src: 'url1',
						image_url: '../../static/temp/banner1.jpg'
					},
					{
						id: 2,
						src: 'url2',
						image_url: '../../static/temp/banner2.jpg'
					}
				],
				// 分类菜单
				categoryList: [{
		...
		methods: {
			...
			toGoods(item) {
				...
				// 	icon: 'none'
				// });
				let url = `/pages/product/product?id=${id}&co_id=${item.co_id}`;
				if (!this.hasLogin) {
					url = '/pages/public/login';
				}
				uni.navigateTo({
