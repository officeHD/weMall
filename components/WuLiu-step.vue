			...
			<block v-for="(item,i) in datas" :key="i">
				<view class="State_Four">
					<view class="FourBox">
						...
						<text class="ali_top_icon" style="margin: 0 0;" v-if="item.AcceptStation.includes(ways.expName) || i == datas.length -1">发</text>
						<!-- =====如果不够，可以再方法和过滤器中同时添加判断-----或者如下的--[v-else-if]操作 -->
						<text class="ali_top_icon" style="margin: 0 0;" v-else-if="item.AcceptStation.includes('丰巢智能快递柜') && i!= datas.length-1">待</text>
						<text class="ali_top_icon" :class="[item.AcceptStation.includes('签收')?'isOk':'']" style="margin: 0 0;" v-else-if="Type(item.AcceptStation) == true && i!= datas.length-1">{{item.AcceptStation | Type}}</text>
